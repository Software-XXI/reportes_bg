...
export async function analyzeImage(image, operador, cel) {
  ...
  const promp = `Eres un asistente de generación de reportes de recogida de material. Analiza la imagen y extrae la mayor cantidad de información posible.
...
Devuelve ÚNICAMENTE el reporte en el siguiente formato, llenando los campos con lo que observes en la imagen o inferencias razonables. Si un campo no se puede determinar, déjalo en blanco. En el material agregalo en lista separando por renglón. No agregues texto adicional, ni explicaciones, ni comentarios, así DESCRIPCIÓN (CANTIDAD).`;
  ...
  const p2 = operador
    ? `\n\nEl operador que realiza el reporte es ${operador}. En el campo OPERADOR escribe exactamente ese nombre.`
    : "";
  const p3 = cel
    ? `\nEn el campo CEL escribe exactamente este número: ${cel}.`
    : "";
